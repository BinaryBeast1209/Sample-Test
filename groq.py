...
def generate_response(prompt:str,temperature:float=0.3,max_tokens:int=512)->str:
    ...
    for model in MODELS:
        try:
            response=c.chat.completions.create(model=model,messages=[{"role":"user","content":prompt}],temperature=temperature,max_tokens=max_tokens)
            return response.choices[0].message.content
        except Exception as e:
            last_err=e
    return(
        "GROQ MODEL FAILED \n"
        f"Tried the models :{MODELS}\n\n",
